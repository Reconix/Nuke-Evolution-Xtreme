...
if (isset($_POST['save'])) {
    ...
    <fieldset><legend>'._EDITHEADLINE.'</legend><form method="post" action="'.$admin_file.'.php?op=headlines">
    ...
    <input type="submit" value="'._SAVECHANGES.'" /></form></fieldset>';
    CloseTable();
    ...
    confirm_msg($admin_file.'.php?op=headlines&amp;del='.intval($_GET['del']), _SURE2DELHEADLINE);
} else {
    include_once(NUKE_BASE_DIR.'header.php');
    GraphicAdmin();
    OpenTable();
    ...
    while (list($hid, $sitename, $headlinesurl) = $db->sql_fetchrow($result)) {
     ...
     <tr'.$bgcolor.'">
       ...
       <td><a href="'.$admin_file.'.php?op=headlines&amp;edit='.$hid.'">'._EDIT.'</a> / <a href="'.$admin_file.'.php?op=headlines&amp;del='.$hid.'">'._DELETE.'</a></td>
     </tr>';
     }
     echo '</table><br /><br />
     <fieldset><legend>'._ADDHEADLINE.'</legend><form method="post" action="'.$admin_file.'.php?op=headlines">
     <label class="ulog" for="xsitename">'._SITENAME.'</label>
      <input type="text" name="xsitename" size="50" maxlength="30" /><br />
     <label class="ulog" for="headlinesurl">'._RSSFILE.'</label>
      <input type="text" name="headlinesurl" size="50" maxlength="200" /><br /><br />
     <input type="submit" name="addHeadline" value="'._ADDHEADLINE.'" /></form></fieldset>';
     CloseTable();
     include_once(NUKE_BASE_DIR.'footer.php');
}
