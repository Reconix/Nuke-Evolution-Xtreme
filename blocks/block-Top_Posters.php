...
if ($nxp_toppost_config["show_link"] == 1)
{
	...
	//$content .= " Sort Top " . $nxp_toppost_config["show_maxi"] . " by: ";
	//$content .= " <br>\n";

	/*if (!eregi("nxp_toppost_post=1", $_SERVER['REQUEST_URI']))
	{
		$request_url = eregi_replace("&nxp_toppost_poin=1", "", $_SERVER['REQUEST_URI']);
		$request_url = eregi_replace("\?nxp_toppost_poin=1", "", $_SERVER['REQUEST_URI']);
		
		if (eregi("\?", $request_url))
		{
			$nxp_toppost_post = $request_url . "&nxp_toppost_post=1";
		}
		else
		{
			$nxp_toppost_post = $request_url . "?nxp_toppost_post=1";
		...
		$content .= "<a href='" . $nxp_toppost_post . "'>Posts</a>";
	}
	else
	{
		$content .= "<u>Posts</u>";
	...
	if (!eregi("nxp_toppost_poin=1", $_SERVER['REQUEST_URI']))
	{
		$request_url = eregi_replace("&nxp_toppost_post=1", "", $_SERVER['REQUEST_URI']);
		$request_url = eregi_replace("\?nxp_toppost_post=1", "", $_SERVER['REQUEST_URI']);
		
		if (eregi("\?", $request_url))
		{
			$nxp_toppost_poin = $request_url . "&nxp_toppost_poin=1";
		}
		else
		{
			$nxp_toppost_poin = $request_url . "?nxp_toppost_poin=1";
		...
		$content .= "<a href='" . $nxp_toppost_poin . "'>Points</a>";
	}	
	else
	{
		$content .= "<u>Points</u>";
...
$Q02 = "SELECT user_id, username, user_avatar, user_from, user_posts, user_rank
	FROM " . $prefix . "_users
	WHERE username != 'Anonymous' 
	ORDER BY " . $nxp_toppost_order_by . "
	LIMIT 0," . $nxp_toppost_show_numb;
...
while($nxp_toppost_user_info = $db->sql_fetchrow($R02))
{
	...
	{
		$nxp_toppost_rank_title = "Unknown Rank";
	}
