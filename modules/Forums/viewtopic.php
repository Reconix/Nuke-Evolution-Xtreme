...
$sql = "SELECT t.topic_id, t.topic_title, t.topic_status, t.topic_replies, t.topic_time, t.topic_type, t.topic_vote, t.topic_last_post_id, f.forum_name, f.forum_status, f.forum_password, f.forum_id, f.auth_view, f.auth_read, f.auth_post, f.auth_reply, f.auth_edit, f.auth_delete, f.auth_sticky, f.auth_announce, f.auth_pollcreate, f.auth_vote, f.auth_attachments" . $count_sql . "
...
/*****[BEGIN]******************************************
 ...
 [ Mod:    Users Reputations System            v1.0.0 ]
 ******************************************************/
$sql = "SELECT u.username, u.user_id, u.user_posts, u.user_from, u.user_from_flag, u.user_website, u.user_birthday, u.birthday_display, u.user_email, u.user_icq, u.user_aim, u.user_facebook, u.user_myspace, u.user_yim, u.user_regdate, u.user_msnm, u.user_viewemail, u.user_rank, u.user_rank2, u.user_rank3, u.user_rank4, u.user_rank5, u.user_sig, u.user_sig_bbcode_uid, u.user_avatar, u.user_avatar_type, u.user_allowavatar, u.user_allowsmile, u.user_allow_viewonline, u.user_session_time, u.user_gender, p.*,  pt.post_text, pt.post_subject, pt.bbcode_uid, u.user_reputation
...
/*****[BEGIN]******************************************
 [ Mod:     Member Country Flags               v2.0.7 ]
 ******************************************************/
		$poster_from_flag = ( $postrow[$i]['user_from_flag'] && $postrow[$i]['user_id'] != ANONYMOUS ) ? "<img src=\"images/flags/" . $postrow[$i]['user_from_flag'] . "\" alt=\"" . $postrow[$i]['user_from_flag'] . "\" border=\"0\" width=\"16\" height=\"11\" /><br />" : "";
/*****[END]********************************************
 [ Mod:     Member Country Flags               v2.0.7 ]
 ******************************************************/
 ...
 ******************************************************/
                ...
                {
                   case 1 : $gender_image = $lang['Gender'].": <img src=\"images/male.png\" alt=\"" . $lang['Gender'].  ":".$lang['Male']."\" title=\"" . $lang['Gender'] . ":".$lang['Male']. "\" border=\"0\" />"; break;
                   case 2 : $gender_image = $lang['Gender'].": <img src=\"images/female.png\" alt=\"" . $lang['Gender']. ":".$lang['Female']. "\" title=\"" . $lang['Gender'] . ":".$lang['Female']. "\" border=\"0\" />"; break;
 ...
 ******************************************************/
		        ...
		        $myspace_img = ( $postrow[$i]['user_myspace'] ) ? '<a href="' . $postrow[$i]['user_myspace'] . '" target="_userwww"><img src="' . $images['icon_myspace'] . '" alt="' . $lang['MYSPACE'] . ': ' . $postrow[$i]['user_myspace'] . '" title="' .                $lang['MYSPACE'] . ': ' . $postrow[$i]['user_myspace'] . '" border="0" /></a>' : '';
		        $myspace = ( $postrow[$i]['user_myspace'] ) ? '<a href="' . $temp_url . '">' . $lang['MYSPACE'] . '</a>' : '';				
		...
		else
		{
			...
			if ($active == 1)
			{
				...
				if ($who == 1)
				{
					...
					{
						message_die(GENERAL_MESSGAE, $msg);
					}
				}
				// New & Old
				else
