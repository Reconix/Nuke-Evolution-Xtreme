...
include($phpbb_root_path .'language/lang_'. $board_config['default_lang'] .'/lang_ftr.'. $phpEx);
	...
	if ($mode == 'users')
	{
		...
		$pagination 	= generate_pagination(append_sid("admin_ftr.$phpEx?mode=users&order=$order&dir=$dir"), count($rows), 100, $start). '&nbsp;';
		$page_number 	= sprintf($lang['Page_of'], ( floor( $start / 100 ) + 1 ), ceil( count($rows) / 100 ) );				
